...
bool canBeMoved(Board *game, int pieceIndex, int moveBy) {
	// Index out of range
	if (pieceIndex >= nPoints || pieceIndex < 0) return false;
...
moveToPoint canMoveTo(Board *game, int fromIndex, int toIndex) {
	...
	if (destinationSize == PAWNS_PER_POINT) return BLOCKED;
	// Check for empty destination Point
	if (destinationSize == 0) return POSSIBLE;
	// Check if Point is occupied by same player, by now we know the point has at least one empty slot
	if (*game->points[toIndex].pawns[0]->owner == *game->points[fromIndex].pawns[0]->owner) return POSSIBLE;
	// Check if Point is blocked by opponent
	if (destinationSize > CAPTURE_THRESHOLD) return BLOCKED;
